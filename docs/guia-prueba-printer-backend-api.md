...
## ⚠️ Si ya rompiste la instalación (error 0xc000012f con `node.dll` / `ffmpeg.dll`)

Lo que te pasó en la captura (`Imagen incorrecta`, `0xc000012f`) normalmente significa que se mezclaron binarios incompatibles en `C:\Program Files\MakerBot\MakerBotPrint` (por ejemplo, copiar archivos del repo encima de la instalación).

### Recuperación rápida (recomendada)

1. **Desinstalá MakerBot Print** desde "Aplicaciones y características".
2. Borrá manualmente la carpeta si quedó:
   - `C:\Program Files\MakerBot\MakerBotPrint`
3. Reiniciá Windows.
4. Reinstalá MakerBot Print desde instalador oficial.
5. Verificá que abre normalmente **antes** de volver a probar cambios.

### Importante para próximas pruebas

- **NO copies** DLL/EXE (`node.dll`, `ffmpeg.dll`, `makerbot-print.exe`, etc.) desde este repo a `Program Files`.
- Para pruebas aisladas usá el bridge + mock (sección 2).
- Si querés testear código real, reemplazá **solo** archivos JS concretos y siempre con backup.
...
Regla de oro: **nunca reemplazar toda la carpeta** de instalación con contenido del repo.
Si necesitás probar un cambio puntual, reemplazá solo el archivo JS específico (ej. `printer-backend-api.js`) y conservá backup del original.
