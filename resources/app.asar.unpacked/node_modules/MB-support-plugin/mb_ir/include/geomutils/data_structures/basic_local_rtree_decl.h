...
#ifndef INCLUDE_GEOMUTILS_DATA_STRUCTURES_BASIC_LOCAL_RTREE_DECL_H_
#define INCLUDE_GEOMUTILS_DATA_STRUCTURES_BASIC_LOCAL_RTREE_DECL_H_
...
template<typename NUM_T, size_t D>
struct to_bbox_fct {
  // AABBox: basic_bounding_box<Scalar, 2>
  template <typename T>
  basic_bounding_box<NUM_T, D> operator()(const T& arg) {
    return to_bbox<T>::bound(arg);
  }
};

template <typename T, size_t DIM = 2,
          typename BBOX_MAKER = to_bbox_fct<Scalar, DIM>,
          size_t C = LRTREE_DEFAULT_BRANCH>
class basic_local_rtree {
 public:
  /// @brief the type of thing this tree contains
  typedef T value_type;
  typedef BBOX_MAKER bbox_maker;
  static const size_t dimensions = DIM;
  typedef basic_bounding_box<Scalar, dimensions>      BBox;

  /// @brief a value and its bounding box are stored together in a pair
  typedef std::pair<BBox, value_type> bound_value;
  class data_element {
 public:
    data_element();
    data_element(const data_element& other);
    data_element(data_element&& other);
    data_element(const value_type& value, const BBox &bb);
    data_element(const value_type& value, size_t index, const BBox &bb);
    data_element(value_type&& value, const BBox &bb);
    data_element(value_type&& value, size_t index, const BBox &bb);
    data_element& operator =(const data_element& other);
    data_element& operator =(data_element&& other);
  ...
  class basic_iterator {
   ...
   private:
    ...
    basic_iterator(OBASE base)  // NOLINT(runtime/explicit)
      : m_base(base) {
    }
   ...
   public:
    ...
    class iterator : public boost::iterator_facade<
     ...
     public:
      ...
      friend class cursor;
      iterator();
      // visual studio does not support explicit defaults
      // iterator(const iterator& other) = default;
      // iterator& operator =(const iterator& other) = default;
      // ~iterator() = default
     private:
      explicit iterator(cursor* cursor_ptr,
          data_const_iterator data_iter = data_const_iterator());
      void increment();
      bool equal(const iterator& other) const;
      const value_type& dereference() const {
        return m_data_iter->m_value.second;
      }
      cursor* m_cursor_ptr;
      data_const_iterator m_data_iter;
    };
    friend class basic_local_rtree;
    iterator begin() {
      return iterator(this, get_next_data());
    }
    iterator end() {
      return iterator(this, m_parent.m_data.end());
    }
    cursor(const cursor& other);
   private:
    cursor(); // = delete;
    cursor& operator =(const cursor&); // = delete;
    explicit cursor(const basic_local_rtree& parent,
        const filter_t& filt = filter_t());
    data_const_iterator get_next_data();

    typedef std::stack<size_t, std::vector<size_t> > cursor_work_stack;
    cursor_work_stack m_work_stack;
    const basic_local_rtree& m_parent;
    filter_t m_filter;
  };
  ...
  basic_local_rtree(const bbox_maker &maker = bbox_maker());
  /// @brief these constructors behave as expected
  basic_local_rtree(const basic_local_rtree& other);
  /// @brief these constructors behave as expected
  basic_local_rtree(basic_local_rtree&& other);
  /// @brief these constructors behave as expected
  basic_local_rtree& operator=(const basic_local_rtree& other);
  /// @brief these constructors behave as expected
  basic_local_rtree& operator=(basic_local_rtree&& other);
  ...
  /*!Insert a value into the spacial index
     @value: an rvalue reference of what should be inserted.
     a copy of this will be stored.
     @return: an iterator to what you just inserted (not implemented)*/
   ...
   */
  void clear();
  /*!Search for values that meet criteria of filt.filter(BBox)
  ...
  /**
   * @brief Search for values that meet criteria of filt.filter(BBox)
   * @param filt object supporting filter(...) that defines the criteria
   * @return A cursor object that supports begin() and end() calls.
   * Iterators returned by these calls iterate over those elements
   * that satisfy the filter condition.
   */
  template <typename FILTER>
  cursor<FILTER> search_cursor(const FILTER& filt) const {
    return cursor<FILTER>(*this, filt);
  }
  ...
  /// Test integrity of data structures, and throw exception on fail
  void validate() const;
  ...
  std::vector<BBox> repr_level(size_t recursionLevel) const {
    return repr_level(recursionLevel, m_root);
  }
  std::vector<BBox> repr_level(size_t recursionLevel, size_t index) const {
    std::vector<BBox> ret;
    if (recursionLevel == 0) {
      ret.push_back(m_nodes[index].bound());
    } else {
      node n = m_nodes[index];
      for (auto it = n.begin(); it != n.end(); ++it) {
        std::vector<BBox> tret = repr_level(recursionLevel - 1, it->index());
        for (uint i = 0; i < tret.size(); ++i)
 ...
 private:
   ...
   public:
    ...
    class basic_iterator {
     public:
      ...
      BASE& operator *() {
        return m_owner->m_parent->dereferenceNode(m_owner->m_children[m_index]);
      }
      BASE* operator ->() {
        return &**this;
      }
    ...
    /// @return an iterator pointing to this node's data and itself
    data_iterator dataIter() const;
    /// @return this node's bounding box
    const BBox& bound() const;
    ...
    /**
       ...
       @param data
     */
    void setData(data_iterator data);
    /**
    ...
    void readjustBounds();
    /**
       @brief adjust my bounds to contain node at limit_index
    ...
    /// @brief iterate over the children
    iterator end() {
      return iterator(this, m_childrenCount);
    }
    /// @brief iterate over the children
    const_iterator begin() const {
    ...
    /// @brief iterate over the children
    const_iterator end() const {
      return const_iterator(this, m_childrenCount);
    }
   ...
   */
  inline node& dereferenceNode(size_t index) {
  ...
  /**
     ...
     @return node at index
   */
  inline const node& dereferenceNode(size_t index) const {
    return m_nodes[index];
  }
  /**
  ...
  /**
     @brief Check that all relational links are valid for this node and
     its children
     Recurses down to each child, and throws exception on fail
   */
  ...
  static size_t DEFAULT_CHILD_PTR() {
    return -1;
  }
  data_const_iterator DEFAULT_DATA_PTR() const {
    return m_data.end();
  }
  data_iterator DEFAULT_DATA_PTR() {
    return m_data.end();
  }
  ...
  size_t m_root;
  bbox_maker m_bbox_maker;
};

class LocalTreeException : public std::runtime_error {
 public:
  LocalTreeException(const std::string str)
 ...
 */
template <typename T, size_t DIM, typename B, size_t C>
void swap(geomutils::basic_local_rtree<T, DIM, B, C>& lhs,
          geomutils::basic_local_rtree<T, DIM, B, C>& rhs) {
  lhs.swap(rhs);
}
}  // namespace std

#endif  // INCLUDE_GEOMUTILS_DATA_STRUCTURES_BASIC_LOCAL_RTREE_DECL_H_
